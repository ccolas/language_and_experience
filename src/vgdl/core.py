...
class SpriteRegistry:
    ...
    def register_sprite_class(self, key, cls, args, stypes):
        assert not key in self.sprite_keys, f'Sprite key already registered: {key}'
    ...
    def generate_id_number(self, key):
        ids = [int(sprite.id.split('.')[-1]) for sprite in self._live_sprites_by_key[key]] +  [int(sprite.id.split('.')[-1]) for sprite in self._dead_sprites_by_key[key]]
        if len(ids) > 0:
            count = np.max(ids)
        else:
            count = 0
    ...
    def get_state(self) -> dict:
        ...
        for sprite_type in self.saveable_keys():
            sprites = self._live_sprites_by_key[sprite_type] + self._dead_sprites_by_key[sprite_type]
    ...
    def reinstate_sprite(self, key, sprite_state: 'SpriteState', block_size):
        ...
        sprite = self.create_sprite(key, sprite_state['id'],
                                    pos=sprite_state['state']['rect'].topleft, size=block_size)
        sprite.set_game_state(sprite_state['state'])
        if not sprite.alive:
            self.kill_sprite(sprite)
    ...
    def copy(self):
        ...
        sprite_registry.stypes = dict(zip(self.stypes.keys(), [el.copy() for el in self.stypes.values()]))
        sprite_registry.sprite_keys = self.sprite_keys.copy()
        sprite_registry.singletons = self.singletons.copy()
        sprite_registry._sprite_by_id = dict(zip(self._sprite_by_id.keys(), [el.copy() for el in self._sprite_by_id.values()]))
...
class BasicGame:
    ...
    def __init__(self, sprite_registry, title=None, block_size=1, seed=np.random.randint(1e6), **kwargs):
    ...
    def build_level(self, lstr):
        ...
        level = BasicGameLevel(self, copy.deepcopy(self.domain_registry),
                               lstr, width=lengths[0], height=len(lines), seed=self.seed)
...
class BasicGameLevel:
    ...
    def quick_build_and_set_state(self, hidden_state):
        ...
        for obj in state_info:
            ...
            id = obj['obj_id']
            key = obj['name']
            pos = (self.round(obj['pos'][0] * self.block_size, decimal=1), self.round(obj['pos'][1] * self.block_size, decimal=1))
            poss.append(pos[1] / self.block_size)
            size = (self.block_size, self.block_size)
            img = obj['img']
            rng = self.random_generator
            sprite = sclass(key=key, id=id, pos=pos, size=size, rng=rng, img=img, resources=obj['resources'].copy(), **args)
            sprite.stypes = stypes
    ...
    def get_hidden_state(self, return_orientation=False):
        hidden_state = dict(i_step=self.time, obj_hidden_state=[], shape=(self.width, self.height), agent_orientation=dict())
        for key in self.sprite_registry.sprite_keys:
            for s in self.sprite_registry._live_sprites_by_key[key]:
                params = s.__dict__.copy()
                for k in ['key', 'name', 'id', 'pos', 'size', 'rng', 'img', 'resources']:
                    if k in params.keys():
                        del params[k]
                obj = dict(type=s.__class__.__name__,
                           params=params,
                           obj_id=s.id,
                           name=key,
    ...
    def create_sprite(self, key, pos, id=None) -> Optional['VGDLSprite']:
        ...
        sprite = self.sprite_registry.create_sprite(key, pos=pos, id=id,
                                                    size=(self.block_size, self.block_size),
                                                    rng=self.random_generator)
        self.is_stochastic = self.domain.is_stochastic or sprite and sprite.is_stochastic or self.is_stochastic
        return sprite
    ...
    def _move_event_handling(self):
        ...
        # apply stepbacks
        stepback_events = [eff for eff in self.domain.collision_eff if eff.name == 'stepBack']
        ...
        # apply movements
        move_events = [eff for eff in self.domain.collision_eff if eff.name in ['bounceForward', 'reverseDirection', 'turnAround']]
    ...
    def apply_effect(self, effect, ss):
        ...
        if g2 == "EOS":
            ...
            for index in range(len(ss1) - 1, -1, -1):
                ...
                if not self.contains_rect(s1.rect):
                    try:
                        event_key = (effect.name, s1.key, 'EOS', (s1.rect.x, s1.rect.y), (None, None))
        ...
        for sprite in sprites:
            ...
            for collision_i in sprite.rect.collidelistall(others):
                ...
                if reverse:
                    if other not in self.kill_list:
                        reverse_event_key = (effect.name, other.key, sprite.key, (other.rect.x, other.rect.y), (sprite.rect.x, sprite.rect.y))
                        self.add_score(effect.score)
                    ...
                    if sprite not in self.kill_list:
                        event_key = (effect.name, sprite.key, other.key, (sprite.rect.x, sprite.rect.y), (other.rect.x, other.rect.y))
                        self.add_score(effect.score)
    ...
    def _event_handling(self, ss):
        all_events_triggered = []
        all_events_triggered_keys = []
        non_move_effects = [eff for eff in self.domain.collision_eff if eff.name not in ['stepBack', 'bounceForward', 'reverseDirection', 'turnAround']]
    ...
    def tick(self, action: Union[Action, int]):
        ...
        for resource_change in self.resource_changes:
            ...
            sprite.resources[resource] = max(0, min(sprite.resources[resource]+value, self.domain.resources_limits.get(resource, float('+inf'))))
    ...
    def _check_terminations(self):
        ...
        for t in self.domain.terminations:
            ...
            if t.__class__.__name__ == 'Timeout':
                pass
            elif t.__class__.__name__ in ['SpriteCounter', 'MultiSpriteCounter']:
    ...
    def contains_rect(self, rect: pygame.Rect):
        game_rect = pygame.Rect((0, 0), self.screensize) #(self.screensize[0] + self.block_size, self.screensize[1] + self.block_size))
...
class VGDLSprite:
    ...
    state_attributes = ['rect', 'alive', 'resources', 'speed', 'orientation', '_age', 'lastmove']
